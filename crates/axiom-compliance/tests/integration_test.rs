...
fn test_log_multiple_tool_invocations() {
    ...
    let tools = vec![
        ("arm-none-eabi-gcc", "14.3.1", vec!["-c", "main.c", "-o", "main.o"]),
        ("arm-none-eabi-gcc", "14.3.1", vec!["-c", "utils.c", "-o", "utils.o"]),
        ("arm-none-eabi-ld", "14.3.1", vec!["main.o", "utils.o", "-o", "app.elf"]),
        ("arm-none-eabi-objcopy", "14.3.1", vec!["-O", "ihex", "app.elf", "app.hex"]),
    ];
    ...
    assert!(!records[0].diagnostics.is_empty(), "First record should have diagnostics");
}
...
fn test_records_contain_correct_checksums() {
    ...
    let mut record = ToolUsageRecord::new(
        ...
        "14.3.1".to_string(),
        vec!["-c".to_string(), "input.c".to_string(), "-o".to_string(), "output.o".to_string()],
        0,
    );
...
fn test_compliance_error_display_annotation_parse() {
    ...
    assert!(display.contains("Invalid requirement format"), "Error should include message");
}
...
fn test_compliance_error_display_requirement_not_found() {
    ...
    assert!(display.contains("REQ-999"), "Error should mention requirement ID");
    assert!(display.contains("not found"), "Error should mention not found");
}
...
fn test_compliance_error_display_coverage_data_not_found() {
    ...
    assert!(display.contains("coverage.gcov"), "Error should mention file");
    assert!(display.contains("not found"), "Error should mention not found");
}
...
fn test_compliance_error_display_invalid_coverage_data() {
    ...
    assert!(display.contains("coverage.gcov"), "Error should mention file");
    assert!(display.contains("Invalid coverage data"), "Error should mention invalid data");
    assert!(display.contains("Unexpected format"), "Error should include details");
}

#[test]
fn test_compliance_error_display_tool_qualification() {
    let error = ComplianceError::tool_qualification_error(
        "Failed to write log entry".to_string()
    );
    let display = format!("{}", error);
    
    assert!(display.contains("Tool qualification"), "Error should mention tool qualification");
    assert!(display.contains("Failed to write"), "Error should include message");
}
...
fn test_compliance_error_display_checksum_mismatch() {
    ...
    assert!(display.contains("abc123"), "Error should show expected checksum");
    assert!(display.contains("def456"), "Error should show found checksum");
    assert!(display.contains("mismatch"), "Error should mention mismatch");
}
...
fn test_compliance_error_display_mode_not_enabled() {
    ...
    assert!(display.contains("not enabled"), "Error should mention not enabled");
}
...
fn test_compliance_error_display_unsupported_export_format() {
    ...
    assert!(display.contains("not supported"), "Error should mention not supported");
}
...
fn test_compliance_error_display_deviations_detected() {
    ...
    assert!(display.contains("2 deviation"), "Error should show deviation count");
    assert!(display.contains("Deviations detected"), "Error should mention deviations");
}
