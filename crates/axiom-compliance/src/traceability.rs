...
///
/// Returns a vector of traceability links found in the file.
...
/// and test files, parsing requirement and test annotations.
///
/// Returns a complete traceability matrix.
...
///
/// Returns a list of functions that lack traceability.
...
/// implementation links but no test links.
///
/// Returns a list of requirement IDs without test coverage.
pub fn find_untested_requirements(matrix: &TraceabilityMatrix) -> Vec<String> {
    ...
    for req_id in all_reqs {
        ...
        let has_impl = links.iter().any(|link| link.link_type == LinkType::Implementation);
...
/// - Created At
///
/// This format is suitable for certification documentation.
